...
class MessageManager:
    ...
    async def add_message_history_safely(
        ...
        if calculated_tokens_to_use is None:
            tokens: int = (
                user_gpt_context.get_tokens_of(content)
                + user_gpt_context.gpt_model.value.token_margin
            )
        else:
            tokens: int = (
                calculated_tokens_to_use + user_gpt_context.gpt_model.value.token_margin
            )
    ...
    async def pop_message_history_safely(
        ...
        try:
            message_history: MessageHistory = getattr(
                user_gpt_context, f"{role}_message_histories"
            ).pop()
        except IndexError:
            return None
        setattr(
            user_gpt_context,
            f"{role}_message_tokens",
            getattr(user_gpt_context, f"{role}_message_tokens")
            - message_history.tokens,
        )
        if rpop:
    ...
    async def set_message_history_safely(
        ...
        role = GptRoles.get_name(role).lower()
        message_history_to_change: MessageHistory = getattr(
            user_gpt_context, f"{role}_message_histories"
        )[index]
